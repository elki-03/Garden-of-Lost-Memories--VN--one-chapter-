...
function loadProgress($user_id, $con) {
    ...
    if (mysqli_num_rows($erg) > 0) {
        return mysqli_fetch_assoc($erg)['aktuelle_textstelle'];
    } else {
...
function loadSavedProgress($user_id, $con) {
    ...
    if (mysqli_num_rows($erg) > 0) {
        ...
        // Falls kein gespeicherter Fortschritt gefunden wurde, gib einen Default-Wert zurück
        return '1_1'; // Beispiel für Kapitel 1, Abschnitt 1
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['naechste_textstelle'])) {
    ...
    // Verwende den geladenen Spielstand
    $aktuelle_textstelle = $gespeicherte_textstelle;
    $_SESSION['textstelle'] = $aktuelle_textstelle;
}
